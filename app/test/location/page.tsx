...
// Fixed to use ONLY icons that exist in grommet-icons package
...
import {
  ...
  Refresh,
  MapLocation,
  Home, // ✅ Using 'Home' instead of 'Hospital' (which doesn't exist)
} from "grommet-icons";
...
export default function EnhancedTestLocationPage() {
  ...
  const validationResult = location
    ? validateLocationWithinRadius(
        testMode === "real"
          ? location
          : testMode === "close"
          ? createTestLocation(hospitalLocation, 50) // 50m away (should pass)
  ...
  return (
    ...
    >
      {/* 📱 PAGE HEADER - Using Home icon instead of Hospital */}
          ...
          <Box gap="medium">
            ...
            <Box>
              ...
              <Select
                options={[
                  {
                    label: "🏥 City General Hospital (NYC)",
                    value: "city-general",
                  },
                  {
                    label: "🏥 Metro Health Center (LA)",
                    value: "metro-health",
                  },
                  {
                    label: "🏥 Downtown Clinic (Chicago)",
        ...
        <Button
          ...
          label={
            loading
              ...
              : "Get My Location & Check Distance"
          }
          primary
          size="large"
      ...
      <Box gap="medium">
          ...
          <Card background="white" elevation="small">
            <CardBody pad="medium">
              <Box direction="row" align="center" gap="small">
                ...
                <Text size="medium">
                  🔍 Getting your location and calculating distance to
                  hospital...
                </Text>
              </Box>
            </CardBody>
        ...
        {location && validationResult && !loading && (
          <Box gap="medium">
                ...
                <Box background="background-back" pad="small" round="small">
                  ...
                  <Text size="small">
                    ...
                    {(testMode === "real"
                      ? location.latitude
                      : testMode === "close"
                      ? createTestLocation(hospitalLocation, 50).latitude
                      : createTestLocation(hospitalLocation, 500).latitude
                    ).toFixed(6)}
                  </Text>
                  <Text size="small">
                    Lon:{" "}
                    {(testMode === "real"
                      ? location.longitude
                      : testMode === "close"
                      ? createTestLocation(hospitalLocation, 50).longitude
                      : createTestLocation(hospitalLocation, 500).longitude
                    ).toFixed(6)}
                  </Text>
                </Box>
              </CardBody>
            ...
            <Card
              background={
                validationResult.isWithinRadius ? "status-ok" : "status-error"
              }
              elevation="small"
            >
              <CardBody pad="medium">
                <Box align="center" gap="small">
                  ...
                  >
                    ...
                    >
                      ...
                      {validationResult.distanceFriendly}
                    </Text>
            ...
            <CardBody pad="medium">
              <Box align="center" gap="small">
                ...
                <Text size="medium" textAlign="center" color="text-weak">
                  Click the button above to test location-based clock-in
                  validation
                </Text>
                <Text size="small" textAlign="center" color="text-weak">
                  💡 This simulates what healthcare workers will experience when
                  clocking in
                </Text>
              </Box>
            </CardBody>
      ...
      <Card background="accent-1" elevation="small" margin={{ top: "large" }}>
        <CardBody pad="small">
          <Text size="small" color="white" textAlign="center">
            🏥 <strong>Healthcare Security:</strong> This ensures workers can
            only clock in when physically present at the facility, preventing
            time theft and ensuring patient care standards.
          </Text>
        </CardBody>
      </Card>
    </Box>
